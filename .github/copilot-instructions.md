...
- **Languages:** Rust (runtime: DSP, realtime audio, CLI, GUI), C++17 (GR blocks), Python 3.10+ (ML training pipeline only).
...
2. **Non-regression gates — never bypass:**
   - **A3 — CW keying transient:** RMS in the first 5 ms of a dit stays within **±1 dB** of raw (`gate_cw_transient`).
   - **A2 — FT8 decode count:** denoised decodes **≥** raw, **zero** false decodes (`gate_ft8_regression`).
...
5. **No allocations or blocking in the audio callback.** Preallocate buffers, reuse FFT plans, hand frames to the worker over bounded try_send channels.
...
- Rust: `cargo fmt`, `cargo clippy --all-targets -- -D warnings`, `cargo test`. C++17 with repo `.clang-format`. Python (train/ only): `ruff format`, `ruff check`, `mypy --strict`.
